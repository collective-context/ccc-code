...
class CCCStackUpgradeController(CementBaseController):
    class Meta:
        label = 'upgrade'
        stacked_on = 'stack'
        stacked_type = 'nested'
        ...
        arguments = [
            ...
            (['--dashboard'],
                dict(help='Upgrade CCC CODE Dashboard', action='store_true')),
            (['--composer'],
            ...
            (['--ngxblocker'],
             dict(help='Upgrade ngxblocker', action='store_true')),
            (['--no-prompt'],
    ...
    def default(self, disp_msg=False):
        ...
        if pargs.wpcli:
            if os.path.isfile('/usr/local/bin/wp'):
                ...
                Log.info(self, "WPCLI is not installed with CCC CODE")
        ...
        if pargs.netdata:
            ...
            if (os.path.isdir('/opt/netdata') or
                ...
                packages = packages + [[
                    f"{CCCVar.netdata_script_url}",
                    '/var/lib/ccc/tmp/kickstart.sh', 'Netdata']]
            else:
        ...
        if pargs.dashboard:
            if (os.path.isfile('/var/www/22222/htdocs/index.php') or
                ...
                Log.info(self, 'CCC CODE dashboard is not installed')
        ...
        # mysqltuner
        if pargs.mysqltuner:
            if CCCAptGet.is_exec(self, 'mysqltuner'):
        ...
        if not apt_packages and not packages:
            ...
            if apt_packages:
                ...
                if os.path.isfile(
                    ...
                    post_pref(self, CCCVar.ccc_nginx, [], True)
                # redis pre_pref
                if "redis-server" in apt_packages:
                    pre_pref(self, CCCVar.ccc_redis)
                # upgrade packages
            ...
            if packages:
                if CCCAptGet.is_selected(self, 'WP-CLI', packages):
                    CCCFileUtils.rm(self, '/usr/local/bin/wp')

                if CCCAptGet.is_selected(self, 'Netdata', packages):
                    CCCFileUtils.rm(self, '/var/lib/ccc/tmp/kickstart.sh')

                if CCCAptGet.is_selected(self, 'ngxblocker', packages):
                    CCCFileUtils.rm(self, '/usr/local/sbin/update-ngxblocker')

                if CCCAptGet.is_selected(self, 'CCC CODE Dashboard', packages):
                    if os.path.isfile('/var/www/22222/htdocs/index.php'):
                        CCCFileUtils.rm(self, '/var/www/22222/htdocs/index.php')
                ...
                if CCCAptGet.is_selected(self, 'Netdata', packages):
                    ...
                    CCCShellExec.cmd_exec(
                        self,
                        "bash /var/lib/ccc/tmp/kickstart.sh "
                ...
                if CCCAptGet.is_selected(self, 'Composer', packages):
                    Log.wait(self, "Upgrading Composer")
                    if CCCShellExec.cmd_exec(
                            self, '/usr/bin/php -v'):
                        CCCShellExec.cmd_exec(
                            self, "php -q /var/lib/ccc"
                ...
                if CCCAptGet.is_selected(self, 'PHPMyAdmin', packages):
                    Log.wait(self, "Upgrading phpMyAdmin")
                    CCCExtract.extract(self, '/var/lib/ccc/tmp/pma.tar.gz',
                                      '/var/lib/ccc/tmp/')
                    shutil.copyfile(('{0}22222/htdocs/db/pma'
                                     '/config.inc.php'
                                     .format(CCCVar.ccc_webroot)),
                                    ('/var/lib/ccc/tmp/phpMyAdmin-{0}'
                                     '-all-languages/config.inc.php'
                                     .format(ccc_phpmyadmin))
                                    )
                    CCCFileUtils.rm(self, '{0}22222/htdocs/db/pma'
                                   .format(CCCVar.ccc_webroot))
                ...
                if os.path.exists('{0}22222/htdocs'.format(CCCVar.ccc_webroot)):
                    CCCFileUtils.chown(self, "{0}22222/htdocs"
                                      .format(CCCVar.ccc_webroot),
                                      'www-data',
                                      'www-data', recursive=True)
