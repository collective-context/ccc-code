...
from ccc.cli.plugins.sitedb import (addNewSite, deleteSiteInfo,
...
class CCCSiteCreateController(CementBaseController):
    class Meta:
        label = 'create'
        stacked_on = 'site'
        stacked_type = 'nested'
    ...
    def default(self):
        ...
        if (pargs.php74 or pargs.php80 or pargs.php81 or
            ...
            data = dict(
                ...
                wpsubdir=False, webroot=ccc_site_webroot)
            data['basic'] = True
        ...
        for version in CCCVar.ccc_php_versions:
            data[version] = False
        ...
        for pargs_version, version in CCCVar.ccc_php_versions.items():
            ...
            if self.app.config.has_section('php'):
                ...
                for ccc_key, php_ver in CCCVar.ccc_php_versions.items():
                    if php_ver == config_php_ver:
                        data[ccc_key] = True
                        data['ccc_php'] = ccc_key
                        php_version = php_ver
                        break
        ...
        try:
            try:
                ...
                doCleanupAction(self, domain=ccc_domain,
            ...
            if 'proxy' in data.keys() and data['proxy']:
                addNewSite(self, ccc_domain, stype, cache, ccc_site_webroot)
            ...
            if 'ccc_db_name' in data.keys() and not data['wp']:
                try:
                    ...
                    updateSiteInfo(self, ccc_domain, db_name=data['ccc_db_name'],
                    ...
                    doCleanupAction(self, domain=ccc_domain,
                                    webroot=data['webroot'],
                                    dbname=data['ccc_db_name'],
                                    dbuser=data['ccc_db_user'],
                                    dbhost=data['ccc_db_host'])
                    deleteSiteInfo(self, ccc_domain)
                    Log.error(self, "Check the log for details: "
                              "`tail /var/log/ccc/ccc.log` "
                              "and please try again")
                ...
                try:
                    cccdbconfig = open("{0}/ccc-config.php"
            ...
            if data['wp']:
                ...
                try:
                    ccc_wp_creds = setupwordpress(self, data, vhostonly)
                    # Add database information for site into database
                    ...
                    doCleanupAction(self, domain=ccc_domain,
                                    ...
                                    dbname=data['ccc_db_name'],
                                    dbuser=data['ccc_db_user'],
                                    dbhost=data['ccc_mysql_grant_host'])
                    deleteSiteInfo(self, ccc_domain)
                    Log.error(self, "Check the log for details: "
                              "`tail /var/log/ccc/ccc.log` "
                              "and please try again")
            ...
            if data['wp'] and (not pargs.vhostonly):
                Log.info(self, Log.ENDC + "WordPress admin user :"
                         " {0}".format(ccc_wp_creds['wp_user']), log=False)
                Log.info(self, Log.ENDC + "WordPress admin password : {0}"
                         .format(ccc_wp_creds['wp_pass']), log=False)
        ...
        if pargs.letsencrypt:
            ...
            if pargs.letsencrypt == "subdomain":
                ...
                if ((ccc_domain_type == 'subdomain')):
                    Log.debug(self, "Domain type = {0}"
                              .format(ccc_domain_type))
                    acme_subdomain = True
                else:
                    acme_subdomain = False
                    acme_wildcard = False
            ...
            if acme_subdomain is True:
                ...
                acme_domains = acme_domains + ['{0}'.format(ccc_domain)]
            ...
            if CCCAcme.cert_check(self, ccc_domain):
                SSL.archivedcertificatehandle(self, ccc_domain, acme_domains)
            else:
                if acme_subdomain is True:
                    ...
                    if SSL.checkwildcardexist(self, ccc_root_domain):
                        ...
                        copyWildcardCert(self, ccc_domain, ccc_root_domain)
                    else:
                        # check DNS records before issuing cert
                        if not acmedata['dns'] is True:
                            if not pargs.force:
                                if not CCCAcme.check_dns(self, acme_domains):
                                    Log.error(self,
                                              "Aborting SSL "
                ...
                if pargs.hsts:
                    SSL.setuphsts(self, ccc_domain)

                SSL.httpsredirect(self, ccc_domain, acme_domains, True)
                SSL.siteurlhttps(self, ccc_domain)
                if not CCCService.reload_service(self, 'nginx'):
                    Log.error(self, "service nginx reload failed. "
                              "check issues with `nginx -t` command")
                Log.info(self, "Congratulations! Successfully Configured "
                         "SSL on https://{0}".format(ccc_domain))
