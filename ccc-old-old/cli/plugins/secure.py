...
from ccc.core.random import RANDOM
...
def ccc_secure_hook(app):
    pass


class CCCSecureController(CementBaseController):
    class Meta:
        label = 'secure'
        stacked_on = 'base'
        stacked_type = 'nested'
    ...
    def secure_auth(self):
        ...
        if not pargs.user_input:
            username = input("Provide HTTP authentication user "
                             "name [{0}] :".format(CCCVar.ccc_user))
            pargs.user_input = username
            if username == "":
                pargs.user_input = CCCVar.ccc_user
        ...
        Log.debug(self, "printf username:"
                  "$(openssl passwd --apr1 "
                  "password 2> /dev/null)\n\""
                  "> /etc/nginx/htpasswd-ccc 2>/dev/null")
        CCCShellExec.cmd_exec(self, "printf \"{username}:"
                             "$(openssl passwd -apr1 "
                             "{password} 2> /dev/null)\n\""
                             "> /etc/nginx/htpasswd-ccc 2>/dev/null"
    ...
    def secure_ip(self):
        ...
        for ip_addr in user_ip:
            if not ("exist_ip_address " + ip_addr in open('/etc/nginx/common/'
                                                          'acl.conf').read()):
                CCCShellExec.cmd_exec(self, "sed -i "
    ...
    def secure_ssh(self):
        ...
        if os.path.isfile('/etc/ssh/sshd_config'):
            ...
            if pargs.allowpassword:
                ccc_allowpassword = 'yes'
            else:
                ccc_allowpassword = 'no'
            data = dict(sshport=current_ssh_port, allowpass=ccc_allowpassword,
                        user=sudo_user)
            CCCTemplate.deploy(self, '/etc/ssh/sshd_config',
                              'sshd.mustache', data)
            CCCGit.add(self, ["/etc/ssh"],
                      msg="Adding changed SSH port into Git")
            if not CCCService.restart_service(self, 'ssh'):
    ...
    def secure_ssh_port(self):
        ...
        if not pargs.user_input:
            ...
            while (not port.isdigit()) and (port != "") and (not port < 65536):
                ...
                port = input("Server SSH port [22]:")
            pargs.user_input = port
        if CCCFileUtils.grepcheck(self, '/etc/ssh/sshd_config', '#Port'):
            CCCShellExec.cmd_exec(self, "sed -i \"s/#Port.*/Port "
...
def load(app):
    app.handler.register(CCCSecureController)
    app.hook.register('post_argument_parsing', ccc_secure_hook)
