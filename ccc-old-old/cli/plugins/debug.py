...
from ccc.cli.plugins.site_functions import logwatch
...
def ccc_debug_hook(app):
    pass


class CCCDebugController(CementBaseController):
    class Meta:
        label = 'debug'
        ...
        usage = "ccc debug [<site_name>] [options] "
    ...
    def debug_nginx(self):
            ...
            for ip_addr in debug_address:
                if not ("debug_connection "+ip_addr in open('/etc/nginx/'
                    ...
                    Log.info(self, "Setting up Nginx debug connection"
                             " for "+ip_addr)
                    CCCShellExec.cmd_exec(self, "sed -i \"/events {{/a\\ \\ \\ "
        ...
        # stop global debug
            ...
            if "debug_connection " in open('/etc/nginx/nginx.conf',
                ...
                CCCShellExec.cmd_exec(self, "sed -i \"/debug_connection.*/d\""
        ...
        # stop site specific debug
            ...
            config_path = ("/etc/nginx/sites-available/{0}"
                           .format(self.app.pargs.site_name))
            if os.path.isfile(config_path):
                if CCCShellExec.cmd_exec(self, "grep \"error.log debug\" {0}"
                                        .format(config_path)):
                    Log.info(self, "Stoping NGINX debug connection for {0}"
                             .format(self.app.pargs.site_name))
                    CCCShellExec.cmd_exec(self, "sed -i \"s/error.log debug;/"
                                         "error.log;/\" {0}"
                                         .format(config_path))
                    self.trigger_nginx = True
    ...
    def debug_php(self):
        ...
        if (self.app.pargs.php == 'on' and not self.app.pargs.site_name):
            if not (CCCShellExec.cmd_exec(self, "sed -n \"/upstream php"
                                               "{/,/}/p \" /etc/nginx/"
                ...
                # Enable xdebug
                CCCFileUtils.searchreplace(self, "/etc/{0}/"
        ...
        # PHP global debug stop
            ...
            if CCCShellExec.cmd_exec(self, " sed -n \"/upstream php {/,/}/p\" "
    ...
    def debug_fpm(self):
        ...
        if (self.app.pargs.fpm == 'on' and not self.app.pargs.site_name):
            if not CCCShellExec.cmd_exec(self, "grep \"log_level = debug\" "
                                              "/etc/{0}/"
        ...
        # PHP5-FPM stop global debug
            ...
            if CCCShellExec.cmd_exec(self, "grep \"log_level = debug\" "
                                          "/etc/{0}/fpm/php-fpm.conf".format("php/7.2")):
    ...
    def debug_php73(self):
        ...
        if (self.app.pargs.php73 ==  'on' and not self.app.pargs.site_name):
            if not (CCCShellExec.cmd_exec(self, "sed -n \"/upstream php73"
                                               "{/,/}/p \" /etc/nginx/"
                                               "conf.d/upstream.conf "
                                               "| grep 9173")):
                ...
                # Enable xdebug
                CCCFileUtils.searchreplace(self, "/etc/php/7.3/mods-available/"
                                          "xdebug.ini",
                                          ";zend_extension",
                                          "zend_extension")
        ...
        # PHP global debug stop
            ...
            if CCCShellExec.cmd_exec(self, " sed -n \"/upstream "
                                    "php73 {/,/}/p\" "
                                          "/etc/nginx/conf.d/upstream.conf "
                                          "| grep 9173"):
    ...
    def debug_mysql(self):
        ...
        if (self.app.pargs.mysql == 'on' and not self.app.pargs.site_name):
            if not CCCShellExec.cmd_exec(self, "mysql -e \"show variables like"
                                              " \'slow_query_log\';\" | "
                                              "grep ON"):
                Log.info(self, "Setting up MySQL slow log")
                CCCMysql.execute(self, "set global slow_query_log = "
                                      "\'ON\';")
                CCCMysql.execute(self, "set global slow_query_log_file = "
                                      "\'/var/log/mysql/mysql-slow.log\';")
                CCCMysql.execute(self, "set global long_query_time = 2;")
                CCCMysql.execute(self, "set global log_queries_not_using"
                                      "_indexes = \'ON\';")
            else:
        ...
        # MySQL stop global debug
            ...
            if CCCShellExec.cmd_exec(self, "mysql -e \"show variables like \'"
                                    "slow_query_log\';\" | grep ON"):
                Log.info(self, "Disabling MySQL slow log")
                CCCMysql.execute(self, "set global slow_query_log = \'OFF\';")
                CCCMysql.execute(self, "set global slow_query_log_file = \'"
                                "/var/log/mysql/mysql-slow.log\';")
                CCCMysql.execute(self, "set global long_query_time = 10;")
                CCCMysql.execute(self, "set global log_queries_not_using_index"
                                "es = \'OFF\';")
                CCCShellExec.cmd_exec(self, "crontab -l | sed \'/#CCC CODE "
    ...
    def debug_wp(self):
        ...
        if (self.app.pargs.wp == 'on' and self.app.pargs.site_name):
            wp_config = ("{0}/{1}/wp-config.php"
                         .format(CCCVar.ccc_webroot,
                                 self.app.pargs.site_name))
            webroot = "{0}{1}".format(CCCVar.ccc_webroot,
                                      self.app.pargs.site_name)
            ...
            if os.path.isfile(wp_config):
                if not CCCShellExec.cmd_exec(self, "grep \"\'WP_DEBUG\'\" {0} |"
                                            " grep true".format(wp_config)):
                    Log.info(self, "Starting WordPress debug")
                    open("{0}/htdocs/wp-content/debug.log".format(webroot),
                         encoding='utf-8', mode='a').close()
                    CCCShellExec.cmd_exec(self, "chown {1}: {0}/htdocs/wp-"
                                         "content/debug.log"
                                         "".format(webroot,
                                                   CCCVar.ccc_php_user))
                    CCCShellExec.cmd_exec(self, "sed -i \"s/define(\'WP_DEBUG\'"
                                         ".*/define(\'WP_DEBUG\', true);\\n"
                                         "define(\'WP_DEBUG_DISPLAY\', false);"
                                         "\\ndefine(\'WP_DEBUG_LOG\', true);"
                                         "\\ndefine(\'SAVEQUERIES\', true);/\""
                                         " {0}".format(wp_config))
                    CCCShellExec.cmd_exec(self, "cd {0}/htdocs/ && wp"
                                         " plugin --allow-root install "
                                         "developer query-monitor"
                                         .format(webroot))
                    CCCShellExec.cmd_exec(self, "chown -R {1}: {0}/htdocs/"
                                         "wp-content/plugins"
                                         .format(webroot,
                                                 CCCVar.ccc_php_user))
            ...
            if not os.path.isfile(wp_config):
                wp_config = ("{0}/{1}/htdocs/wp-config.php"
                             .format(CCCVar.ccc_webroot,
                                     self.app.pargs.site_name))
            if os.path.isfile(wp_config):
                if CCCShellExec.cmd_exec(self, "grep \"\'WP_DEBUG\'\" {0} | "
                                        "grep true".format(wp_config)):
                    Log.info(self, "Disabling WordPress debug")
                    CCCShellExec.cmd_exec(self, "sed -i \"s/define(\'WP_DEBUG\'"
                                         ", true);/define(\'WP_DEBUG\', "
                                         "false);/\" {0}".format(wp_config))
                    CCCShellExec.cmd_exec(self, "sed -i \"/define(\'"
    ...
    def debug_rewrite(self):
        ...
        if (self.app.pargs.rewrite == 'on' and not self.app.pargs.site_name):
            if not CCCShellExec.cmd_exec(self, "grep \"rewrite_log on;\" "
                                        "/etc/nginx/nginx.conf"):
                Log.info(self, "Setting up Nginx rewrite logs")
                CCCShellExec.cmd_exec(self, "sed -i \'/http {/a \\\\t"
        ...
        # Stop Nginx rewrite debug globally
            ...
            if CCCShellExec.cmd_exec(self, "grep \"rewrite_log on;\" "
                                    "/etc/nginx/nginx.conf"):
                Log.info(self, "Disabling Nginx rewrite logs")
                CCCShellExec.cmd_exec(self, "sed -i \"/rewrite_log.*/d\""
                                     " /etc/nginx/nginx.conf")
                self.trigger_nginx = True
            else:
        ...
        # Start Nginx rewrite for site
            ...
            config_path = ("/etc/nginx/sites-available/{0}"
                           .format(self.app.pargs.site_name))
            if not CCCShellExec.cmd_exec(self, "grep \"rewrite_log on;\" {0}"
                                        .format(config_path)):
                Log.info(self, "Setting up Nginx rewrite logs for {0}"
                         .format(self.app.pargs.site_name))
                CCCShellExec.cmd_exec(self, "sed -i \"/access_log/i \\\\\\t"
                                     "rewrite_log on;\" {0}"
                                     .format(config_path))
                self.trigger_nginx = True
            else: Log.info(self, "Nginx rewrite logs for {0} already setup"
                         .format(self.app.pargs.site_name))

            if ('{0}{1}/logs/error.log'.format(CCCVar.ccc_webroot,
                                               self.app.pargs.site_name)
                    not in self.msg):
                self.msg = self.msg + ['{0}{1}/logs/error.log'
                                       .format(CCCVar.ccc_webroot,
        ...
        # Stop Nginx rewrite for site
            ...
            config_path = ("/etc/nginx/sites-available/{0}"
                           .format(self.app.pargs.site_name))
            if CCCShellExec.cmd_exec(self, "grep \"rewrite_log on;\" {0}"
                                    .format(config_path)):
                Log.info(self, "Disabling Nginx rewrite logs for {0}"
                         .format(self.app.pargs.site_name))
                CCCShellExec.cmd_exec(self, "sed -i \"/rewrite_log.*/d\" {0}"
                                     .format(config_path))
                self.trigger_nginx = True
            else:
    ...
    def signal_handler(self, app, signal, frame):
        ...
        # Reload PHP
        if self.trigger_php:
            if CCCAptGet.is_installed(self, 'php7.2-fpm'):
                CCCService.reload_service(self, 'php7.2-fpm')
            if CCCAptGet.is_installed(self, 'php7.3-fpm'):
                CCCService.reload_service(self, 'php7.3-fpm')
        self.app.close(0)
    ...
    def default(self):
        ...
        if ((not self.app.pargs.nginx) and (not self.app.pargs.php) and
            ...
            if self.app.pargs.stop or self.app.pargs.start:
                print("--start/stop option is deprecated since ccc v3.0.5")
        ...
        if self.app.pargs.interval:
            ...
            try:
                if not CCCShellExec.cmd_exec(self, "crontab -l | grep "
                                            "'ccc debug --import-slow-log'"):
                    if not cron_time == 0:
        ...
        if self.trigger_php:
            if CCCAptGet.is_installed(self, 'php7.2-fpm'):
                CCCService.restart_service(self, 'php7.2-fpm')
            if CCCAptGet.is_installed(self, 'php7.3-fpm'):
                CCCService.restart_service(self, 'php7.3-fpm')
    ...
    def import_slow_log(self):
        ...
        if os.path.isdir("{0}22222/htdocs/db/anemometer"
            ...
            if os.path.isfile("/var/log/mysql/mysql-slow.log"):
                ...
                try:
                    CCCShellExec.cmd_exec(self, "pt-query-digest --user={0} "
                                         "--password={1} "
            ...
            Log.error(self, "Anemometer is not installed." +
                      ...
                      Log.BOLD + "\n `ccc stack install --utils`" +
                      Log.ENDC)


def load(app):
    # register the plugin class.. this only happens if the plugin is enabled
    app.handler.register(CCCDebugController)
    # register a hook (function) to run after arguments are parsed.
    app.hook.register('post_argument_parsing', ccc_debug_hook)
