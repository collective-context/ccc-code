...
class CCCMysql():
    ...
    def execute(self, statement, errormsg='', log=True):
        ...
        # & Execute MySQL query
        connection = CCCMysql.connect(self)
    ...
    def backupAll(self, fulldump=False):
        ...
        try:
            Log.info(self, "Backing up database at location: "
                     "/var/lib/ccc-backup/mysql")
            # Setup Nginx common directory
            if not os.path.exists('/var/lib/ccc-backup/mysql'):
                Log.debug(self, 'Creating directory'
                          '/var/lib/ccc-backup/mysql')
                os.makedirs('/var/lib/ccc-backup/mysql')
            if not fulldump:
                ...
                for dbs in db:
                    ...
                    p2 = subprocess.Popen(
                        "/usr/bin/zstd -c > "
                        "/var/lib/ccc-backup/mysql/{0}{1}.sql.zst"
                        .format(dbs, CCCVar.ccc_date),
                ...
                p2 = subprocess.Popen(
                    "/usr/bin/zstd -c > "
                    "/var/lib/ccc-backup/mysql/fulldump-{0}.sql.zst"
                    .format(CCCVar.ccc_date),
                    stdin=p1.stdout, shell=True)
                p1.stdout.close()
                output = p1.stderr.read()
                p1.wait()
    ...
    def check_db_exists(self, db_name):
        try:
            if CCCMysql.dbConnection(self, db_name):
                return True
    ...
    def mariadb_ping(self):
        if os.path.exists('/usr/bin/mariadb-admin'):
            ...
            return False
        if CCCShellExec.cmd_exec(self, f"{mariadb_admin} ping"):
            return True
        else:
