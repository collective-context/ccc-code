...
def ccc_info_hook(app):
    pass


class CCCInfoController(CementBaseController):
    class Meta:
        label = 'info'
        stacked_on = 'base'
        stacked_type = 'nested'
    ...
    def info_php(self):
        ...
        if CCCAptGet.is_installed(self, 'php7.4-fpm'):
            pargs.php74 = True
        else:
            Log.info(self, "PHP 7.4 is not installed")
        if CCCAptGet.is_installed(self, 'php8.0-fpm'):
            pargs.php80 = True
        else:
            Log.info(self, "PHP 8.0 is not installed")
        if CCCAptGet.is_installed(self, 'php8.1-fpm'):
            pargs.php81 = True
        else:
            Log.info(self, "PHP 8.1 is not installed")
        if CCCAptGet.is_installed(self, 'php8.2-fpm'):
            pargs.php82 = True
        else:
            Log.info(self, "PHP 8.2 is not installed")
        if CCCAptGet.is_installed(self, 'php8.3-fpm'):
            pargs.php83 = True
        else:
            Log.info(self, "PHP 8.3 is not installed")
        if CCCAptGet.is_installed(self, 'php8.4-fpm'):
            pargs.php84 = True
        else:
    ...
    def default(self):
        ...
        if pargs.nginx:
            if ((not CCCAptGet.is_installed(self, 'nginx-custom')) and
...
def load(app):
    ...
    app.handler.register(CCCInfoController)

    # register a hook (function) to run after arguments are parsed.
    app.hook.register('post_argument_parsing', ccc_info_hook)
