...
class CCCLogShowController(CementBaseController):
    class Meta:
        label = 'show'
        ...
        usage = "ccc log show [<site_name>] [options]"
                ...
                if os.path.isdir('{0}/htdocs/wp-content'.format(webroot)):
                    if not os.path.isfile('{0}/logs/debug.log'
                        ...
                        if not os.path.isfile('{0}/htdocs/wp-content/debug.log'
                            ...
                            CCCShellExec.cmd_exec(self, "chown {1}: {0}/htdocs/"
                    ...
                    CCCFileUtils.create_symlink(self, ["{0}/htdocs/wp-content/"
                                                      "debug.log"
                                                      .format(webroot),
                                                      '{0}/logs/debug.log'
                                                      .format(webroot)])
...
class CCCLogResetController(CementBaseController):
    class Meta:
        label = 'reset'
        ...
        usage = "ccc log reset [<site_name>] [options]"
    ...
    def default(self):
        ...
        if self.app.pargs.slow_log_db:
            if os.path.isdir("/var/www/22222/htdocs/db/anemometer"):
                ...
                CCCMysql.execute(self, "TRUNCATE TABLE  "
...
class CCCLogGzipController(CementBaseController):
    class Meta:
        label = 'gzip'
        ...
        arguments = [
            ...
            (['--fpm'],
                dict(help='GZip PHP-FPM slow logs file',
                     action='store_true')),
            (['--mysql'],
                dict(help='G Zip MySQL logs file', action='store_true')),
            (['--wp'],
        ...
        usage = "ccc log gzip [<site_name>] [options]"
...
class CCCLogMailController(CementBaseController):
    class Meta:
        label = 'mail'
        ...
        usage = "ccc log mail [<site_name>] [options]"
    ...
    def default(self):
        ...
        for tomail in self.app.pargs.to:
            ...
            CCCSendMail("ccccode", tomail[0], "{0} Log Files"
                       .format(CCCVar.ccc_fqdn),
...
def load(app):
    ...
    app.handler.register(CCCLogController)
    app.handler.register(CCCLogShowController)
    app.handler.register(CCCLogResetController)
    app.handler.register(CCCLogGzipController)
    app.handler.register(CCCLogMailController)
    # register a hook (function) to run after arguments are parsed.
    app.hook.register('post_argument_parsing', ccc_log_hook)
