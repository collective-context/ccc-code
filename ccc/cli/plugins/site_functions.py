...
from ccc.cli.plugins.sitedb import getSiteInfo
from ccc.cli.plugins.stack import CCCStackController
from ccc.cli.plugins.stack_pref import post_pref
...
from ccc.core.mysql import (MySQLConnectionError, StatementExcecutionError,
                           CCCMysql)
...
def setupdomain(self, data):
    ...
    ccc_domain_name = data['site_name']
    ccc_site_webroot = data['webroot']
    ...
    try:
        ccc_site_nginx_conf = open('/etc/nginx/sites-available/{0}'
                                  .format(ccc_domain_name), encoding='utf-8',
                                  mode='w')
        self.app.render((data), 'virtualconf.mustache',
                        out=ccc_site_nginx_conf)
        ccc_site_nginx_conf.close()
    except IOError as e:
    ...
    CCCFileUtils.create_symlink(self, ['/etc/nginx/sites-available/{0}'
                                      .format(ccc_domain_name),
    ...
    try:
        if not os.path.exists('{0}/htdocs'.format(ccc_site_webroot)):
            os.makedirs('{0}/htdocs'.format(ccc_site_webroot))
        if not os.path.exists('{0}/logs'.format(ccc_site_webroot)):
            os.makedirs('{0}/logs'.format(ccc_site_webroot))
        if not os.path.exists('{0}/conf/nginx'.format(ccc_site_webroot)):
            os.makedirs('{0}/conf/nginx'.format(ccc_site_webroot))

        CCCFileUtils.create_symlink(self, ['/var/log/nginx/{0}.access.log'
                                          .format(ccc_domain_name),
                                          '{0}/logs/access.log'
                                          .format(ccc_site_webroot)])
        CCCFileUtils.create_symlink(self, ['/var/log/nginx/{0}.error.log'
                                          .format(ccc_domain_name),
                                          '{0}/logs/error.log'
                                          .format(ccc_site_webroot)])
    except Exception as e:
        ...
        if (os.path.exists('{0}/htdocs'.format(ccc_site_webroot)) and
                os.path.exists('{0}/logs'.format(ccc_site_webroot))):
            Log.info(self, "[" + Log.ENDC + "Done" + Log.OKBLUE + "]")
        else:
...
def setupdatabase(self, data):
    ccc_domain_name = data['site_name']
    ccc_random_pass = (''.join(random.sample(string.ascii_uppercase +
                                            string.ascii_lowercase +
                                            string.digits, 24)))
    ccc_replace_dash = ccc_domain_name.replace('-', '_')
    ccc_replace_dot = ccc_replace_dash.replace('.', '_')
    ccc_replace_underscore = ccc_replace_dot.replace('_', '')
    if self.app.config.has_section('mysql'):
        ...
        ccc_mysql_grant_host = self.app.config.get('mysql', 'grant-host')
    else:
        prompt_dbname = False
        prompt_dbuser = False
    ...
    if prompt_dbname == 'True' or prompt_dbname == 'true':
        try:
            ccc_db_name = input('Enter the MySQL database name [{0}]: '
                               .format(ccc_replace_dot))
        except EOFError:
            raise SiteError("Unable to input database name")

    if not ccc_db_name:
        ccc_db_name = (ccc_replace_dot[0:32] + '_' + generate_8_random())

    if prompt_dbuser == 'True' or prompt_dbuser == 'true':
        try:
            ccc_db_username = input('Enter the MySQL database user name [{0}]: '
                                   .format(ccc_replace_dot))
            ccc_db_password = getpass.getpass(prompt='Enter the MySQL database'
                                             ' password [{0}]: '
                                             .format(ccc_random_pass))
        except EOFError:
            raise SiteError("Unable to input database credentials")

    if not ccc_db_username:
        ccc_db_username = (ccc_replace_underscore[0:12] + generate_random())
    if not ccc_db_password:
        ccc_db_password = ccc_random_pass

    # create MySQL database
    Log.info(self, "Setting up database\t\t", end='')
    Log.debug(self, "Creating database {0}".format(ccc_db_name))
    try:
        if CCCMysql.check_db_exists(self, ccc_db_name):
            Log.debug(self, "Database already exists, Updating DB_NAME .. ")
            ccc_db_name = (ccc_db_name[0:32] + '_' + generate_8_random())
            ccc_db_username = (ccc_db_name[0:12] + generate_random())
    except MySQLConnectionError:
        raise SiteError("MySQL Connectivity problem occured")

    try:
        CCCMysql.execute(self, "create database `{0}`"
                        .format(ccc_db_name))
    ...
    # Create MySQL User
    Log.debug(self, "Creating user {0}".format(ccc_db_username))
    Log.debug(self, "create user `{0}`@`{1}` identified by ''"
              .format(ccc_db_username, ccc_mysql_grant_host))
    try:
        CCCMysql.execute(self,
                        "create user `{0}`@`{1}` identified by '{2}'"
                        .format(ccc_db_username, ccc_mysql_grant_host,
                                ccc_db_password), log=False)
    except StatementExcecutionError:
        Log.info(self, "[" + Log.ENDC + Log.FAIL + "Failed" + Log.OKBLUE + "]")
    ...
    try:
        CCCMysql.execute(self,
                        ...
                        "trigger on `{0}`.* to `{1}`@`{2}`"
                        .format(ccc_db_name,
                                ccc_db_username, ccc_mysql_grant_host))
    except StatementExcecutionError:
        Log.info(self, "[" + Log.ENDC + Log.FAIL + "Failed" + Log.OKBLUE + "]")
    ...
    data['ccc_db_name'] = ccc_db_name
    data['ccc_db_user'] = ccc_db_username
    data['ccc_db_pass'] = ccc_db_password
    data['ccc_db_host'] = CCCVar.ccc_mysql_host
    data['ccc_mysql_grant_host'] = ccc_mysql_grant_host
    return (data)
...
def setupwordpress(self, data, vhostonly=False):
    ...
    if self.app.config.has_section('wordpress'):
        ...
        ccc_wp_user = self.app.config.get('wordpress', 'user')
        ccc_wp_pass = self.app.config.get('wordpress', 'password')
        ccc_wp_email = self.app.config.get('wordpress', 'email')
    else:
    ...
    if 'wp-user' in data.keys() and data['wp-user']:
        ccc_wp_user = data['wp-user']
    if 'wp-email' in data.keys() and data['wp-email']:
        ccc_wp_email = data['wp-email']
    if 'wp-pass' in data.keys() and data['wp-pass']:
        ccc_wp_pass = data['wp-pass']

    Log.info(self, "Downloading WordPress \t\t", end='')
    CCCFileUtils.chdir(self, '{0}/htdocs/'.format(ccc_site_webroot))
    try:
        if CCCShellExec.cmd_exec(self, "wp --allow-root core"
                                " download"):
            pass
        else:
    ...
    if not (data['ccc_db_name'] and data['ccc_db_user'] and data['ccc_db_pass']):
        data = setupdatabase(self, data)
    if prompt_wpprefix == 'True' or prompt_wpprefix == 'true':
        try:
            ccc_wp_prefix = input('Enter the WordPress table prefix [wp_]: ')
            while not re.match('^[A-Za-z0-9_]*$', ccc_wp_prefix):
    ...
    if not data['multisite']:
        ...
        Log.debug(self, "/bin/bash -c \"{0} --allow-root "
                  .format(CCCVar.ccc_wpcli_path) +
                  "config create " +
                  "--dbname=\'{0}\' --dbprefix=\'{1}\' --dbuser=\'{2}\' "
                  "--dbhost=\'{3}\' "
                  .format(data['ccc_db_name'], ccc_wp_prefix,
                          data['ccc_db_user'], data['ccc_db_host']) +
                  "--dbpass= "
                  "--extra-php<<PHP \n {0}\nPHP\""
                  .format("\n\ndefine(\'WP_DEBUG\', false);"))
        try:
            if CCCShellExec.cmd_exec(self, "/bin/bash -c \"{0} --allow-root"
                                    .format(CCCVar.ccc_wpcli_path) +
                                    " config create " +
                                    "--dbname=\'{0}\' --dbprefix=\'{1}\' "
                                    "--dbuser=\'{2}\' --dbhost=\'{3}\' "
        ...
        Log.debug(self, "/bin/bash -c \"{0} --allow-root "
                  ...
                  .format(data['ccc_db_name'],
                          ccc_wp_prefix, data['ccc_db_host']) +
                  "--dbuser=\'{0}\' --dbpass= "
                  "--extra-php<<PHP \n {1} {2} \nPHP\""
                  .format(data['ccc_db_user'],
    ...
    wp_conf_variables = [
        ['WP_REDIS_PREFIX', '{0}:'.format(ccc_domain_name)],
    ...
    # CCCFileUtils.mvfile(self, os.getcwd()+'/wp-config.php',
    ...
    if not ccc_wp_user:
        ccc_wp_user = CCCVar.ccc_user
        while not ccc_wp_user:
            Log.warn(self, "Username can have only alphanumeric"
                     "characters, spaces, underscores, hyphens,"
                     "periods and the @ symbol.")
            try:
                ccc_wp_user = input('Enter WordPress username: ')
            except EOFError:
                raise SiteError("input WordPress username failed")
    if not ccc_wp_pass:
        ccc_wp_pass = ccc_random_pass

    if not ccc_wp_email:
        ccc_wp_email = CCCVar.ccc_email
        while not ccc_wp_email:
            try:
                ccc_wp_email = input('Enter WordPress email: ')
            except EOFError:
                raise SiteError("input WordPress username failed")
    ...
    try:
        while not re.match(r"^[A-Za-z0-9\.\+_-]+@[A-Za-z0-9\._-]+\.[a-zA-Z]*$",
            ...
            ccc_wp_email = input("Enter your email: ")
    except EOFError:
    ...
    if not data['multisite']:
        ...
        Log.debug(
            self, "{0} --allow-root core install "
                  ...
                  .format(data['site_name'], ccc_wp_user) +
        ...
        try:
            if CCCShellExec.cmd_exec(
                ...
                "--admin_password=\'{0}\' "
                "--admin_email=\'{1}\'"
                .format(ccc_wp_pass, ccc_wp_email),
                    log=False):
                pass
            else:
    ...
    try:
        CCCShellExec.cmd_exec(self, " {0} --allow-root "
                             .format(CCCVar.ccc_wpcli_path) +
                             "rewrite structure "
                             "/%postname%/")
    except CommandExecutionError as e:
    ...
    if vhostonly:
        try:
            CCCShellExec.cmd_exec(self, "/bin/bash -c \"{0} --allow-root "
                                 .format(CCCVar.ccc_wpcli_path) +
                                 "db clean --yes\"")
            CCCFileUtils.chdir(self, '{0}'.format(ccc_site_webroot))
            CCCFileUtils.rm(self, "{0}/htdocs".format(ccc_site_webroot))
            CCCFileUtils.mkdir(self, "{0}/htdocs".format(ccc_site_webroot))
            CCCFileUtils.chown(self, "{0}/htdocs".format(ccc_site_webroot),
                              'www-data', 'www-data')
        except CommandExecutionError:
            raise SiteError("Cleaning WordPress install failed")

    wp_creds = dict(wp_user=ccc_wp_user, wp_pass=ccc_wp_pass,
                    wp_email=ccc_wp_email)
...
def setupwordpressnetwork(self, data):
    ...
    try:
        if CCCShellExec.cmd_exec(self, 'wp --allow-root core multisite-convert'
                                ' --title=\'{0}\' {subdomains}'
...
def installwp_plugin(self, plugin_name, data):
    ...
    CCCFileUtils.chdir(self, '{0}/htdocs/'.format(ccc_site_webroot))
    try:
        CCCShellExec.cmd_exec(self, "{0} plugin "
                             .format(CCCVar.ccc_wpcli_path) +
                             "--allow-root install "
...
def setwebrootpermissions(self, webroot):
    ...
    try:
        CCCFileUtils.findBrokenSymlink(self, f'{webroot}')
        CCCFileUtils.chown(self, webroot, CCCVar.ccc_php_user,
                          CCCVar.ccc_php_user, recursive=True)
    except Exception as e:
        Log.debug(self, str(e))
...
def sitebackup(self, data):
    ccc_site_webroot = data['webroot']
    backup_path = ccc_site_webroot + '/backup/{0}'.format(CCCVar.ccc_date)
    if not CCCFileUtils.isexist(self, backup_path):
        CCCFileUtils.mkdir(self, backup_path)
    Log.info(self, "Backup location : {0}".format(backup_path))
    CCCFileUtils.copyfile(self, '/etc/nginx/sites-available/{0}'
    ...
    if data['currsitetype'] in ['html', 'php', 'php72', 'php74',
        ...
        if not data['wp']:
            ...
            CCCFileUtils.copyfiles(self, ccc_site_webroot +
                                  '/htdocs', backup_path + '/htdocs')
            Log.info(self, "[" + Log.ENDC + "Done" + Log.OKBLUE + "]")
        else:
            Log.info(self, "Backing up Webroot \t\t", end='')
            CCCFileUtils.mvfile(self, ccc_site_webroot + '/htdocs', backup_path)
            Log.info(self, "[" + Log.ENDC + "Done" + Log.OKBLUE + "]")
    ...
    if not configfiles:
        ...
        if data['currsitetype'] in ['mysql']:
            ...
            Log.debug(self, "Searching wp-config.php in {0}/htdocs/ "
                      .format(ccc_site_webroot))
            configfiles = glob.glob(ccc_site_webroot + '/htdocs/wp-config.php')

    # if configfiles and CCCFileUtils.isexist(self, configfiles[0]):
    #     ccc_db_name = (CCCFileUtils.grep(self, configfiles[0],
    #                   'DB_NAME').split(',')[1]
    ...
    if data['ccc_db_name']:
        ...
        try:
            ...
            raise SiteError("mysqldump failed to backup database")
        Log.info(self, "[" + Log.ENDC + "Done" + Log.OKBLUE + "]")
        # move wp-config.php/ccc-config.php to backup
        if data['currsitetype'] in ['mysql', 'proxy']:
            if data['php73'] is True and not data['wp']:
                CCCFileUtils.copyfile(self, configfiles[0], backup_path)
            else:
                CCCFileUtils.mvfile(self, configfiles[0], backup_path)
        else:
            CCCFileUtils.copyfile(self, configfiles[0], backup_path)
...
def site_package_check(self, stype):
    ...
    if stype in ['html', 'proxy', 'php', 'mysql', 'wp', 'wpsubdir',
        ...
        if not (CCCAptGet.is_installed(self, 'nginx-custom') or
                CCCAptGet.is_installed(self, 'nginx-mainline')):
            # check if Server has nginx-plus installed
            if CCCAptGet.is_installed(self, 'nginx-plus'):
                # do something
                # do post nginx installation configuration
                Log.info(self, "NGINX PLUS Detected ...")
                apt = ["nginx-plus"] + CCCVar.ccc_nginx
                # apt_packages = apt_packages + CCCVar.ccc_nginx
                post_pref(self, apt, packages)
            elif CCCAptGet.is_installed(self, 'nginx'):
                Log.info(self, "CCC CODE detected a previously"
                               "installed Nginx package. "
                               "It may or may not have required modules. "
                               "\nIf you need help, please create an issue at "
                               "https://github.com/collective-context/ccc-code/issues/ \n")
                apt = ["nginx"] + CCCVar.ccc_nginx
                # apt_packages = apt_packages + CCCVar.ccc_nginx
                post_pref(self, apt, packages)
        ...
        if len(selected_versions) > 1:
            Log.error(self, "Error: two different PHP versions cannot be "
                      "combined within the same CCC CODE site")
    ...
    if ((not pargs.php74) and (not pargs.php80) and
        ...
        for version_key, version_number in CCCVar.ccc_php_versions.items():
            if (self.app.config.has_section('php') and
                ...
                if not CCCAptGet.is_installed(self, f'php{version_number}-fpm'):
                    apt_packages += getattr(
                        CCCVar, f'ccc_{version_key}') + CCCVar.ccc_php_extra
    ...
    for version_key, version_number in CCCVar.ccc_php_versions.items():
        if getattr(pargs, version_key) and stype in [version_key, 'mysql', 'wp', 'wpsubdir', 'wpsubdomain']:
            ...
            if not CCCAptGet.is_installed(self, f'php{version_number}-fpm'):
                apt_packages += getattr(CCCVar, f'ccc_{version_key}') + CCCVar.ccc_php_extra
...
def updatewpuserpassword(self, ccc_domain, ccc_site_webroot):

    ccc_wp_user = ''
    ccc_wp_pass = ''
    CCCFileUtils.chdir(self, '{0}/htdocs/'.format(ccc_site_webroot))

    if not CCCShellExec.cmd_exec(self, "wp --allow-root core"
                                " is-installed"):
        # Exit if ccc_domain is not wordpress install
        Log.error(self, "{0} does not seem to be a WordPress site"
                  .format(ccc_domain))

    try:
        ccc_wp_user = input("Provide WordPress user name [admin]: ")
    except Exception as e:
        Log.debug(self, str(e))
    ...
    if ccc_wp_user == "?":
        ...
        try:
            CCCShellExec.cmd_exec(self, "wp --allow-root user list "
    ...
    try:
        is_user_exist = CCCShellExec.cmd_exec(self, "wp --allow-root user list "
    ...
    if is_user_exist:
        try:
            ccc_wp_pass = getpass.getpass(prompt="Provide password for "
                                         "{0} user: "
                                         .format(ccc_wp_user))

            while not ccc_wp_pass:
                ccc_wp_pass = getpass.getpass(prompt="Provide password for "
                                             "{0} user: "
                                             .format(ccc_wp_user))
        except Exception as e:
        ...
        try:
            CCCShellExec.cmd_exec(self, "wp --allow-root user update {0}"
                                 "  --user_pass={1}"
                                 .format(ccc_wp_user, ccc_wp_pass))
        except CommandExecutionError as e:
            Log.debug(self, str(e))
...
def logwatch(self, logfiles):
    ...
    import base64
    import time
    from ccc.core.logwatch import LogWatcher
...
def generate_random_pass():
    ccc_random10 = (''.join(random.sample(string.ascii_uppercase +
                                         string.ascii_lowercase +
                                         string.digits, 24)))
    return ccc_random10
...
def generate_8_random():
    ccc_random8 = (''.join(random.sample(string.ascii_uppercase +
                                        string.ascii_lowercase +
                                        string.digits, 8)))
    return ccc_random8
...
def deleteDB(self, dbname, dbuser, dbhost, exit=True):
    try:
        ...
        try:
            if CCCMysql.check_db_exists(self, dbname):
        ...
        if dbuser != 'root':
            ...
            try:
                CCCMysql.execute(self, "flush privileges")
            except StatementExcecutionError as e:
                Log.debug(self, str(e))
                Log.debug(self, "drop database failed")
                Log.info(self, "Database {0} not dropped".format(dbname))
...
def removeNginxConf(self, domain):
    if os.path.isfile('/etc/nginx/sites-available/{0}'
        ...
        CCCFileUtils.rm(self, '/etc/nginx/sites-enabled/{0}'
                       .format(domain))
        CCCFileUtils.rm(self, '/etc/nginx/sites-available/{0}'
                       .format(domain))
...
def doCleanupAction(self, domain='', webroot='', dbname='', dbuser='',
    ...
    if domain:
        if os.path.isfile('/etc/nginx/sites-available/{0}'
            ...
            CCCAcme.removeconf(self, domain)
...
def copyWildcardCert(self, ccc_domain_name, ccc_root_domain):

    if os.path.isfile("/var/www/{0}/conf/nginx/ssl.conf"
                      .format(ccc_root_domain)):
        try:
            if not os.path.isdir("/etc/letsencrypt/shared"):
                CCCFileUtils.mkdir(self, "/etc/letsencrypt/shared")
            if not os.path.isfile("/etc/letsencrypt/shared/{0}.conf"
                                  .format(ccc_root_domain)):
                CCCFileUtils.copyfile(self, "/var/www/{0}/conf/nginx/ssl.conf"
                                     .format(ccc_root_domain),
                                     "/etc/letsencrypt/shared/{0}.conf"
                                     .format(ccc_root_domain))
            CCCFileUtils.create_symlink(self, ["/etc/letsencrypt/shared/"
                                              "{0}.conf"
                                              .format(ccc_root_domain),
...
def renewLetsEncrypt(self, ccc_domain_name):

    ssl = CCCShellExec.cmd_exec(
    ...
    if not ssl:
        Log.error(self, "ERROR : Let's Encrypt certificate renewal FAILED!",
                  False)
        if (SSL.getexpirationdays(self, ccc_domain_name) > 0):
            Log.error(self, "Your current certificate will expire within " +
                      str(SSL.getexpirationdays(self, ccc_domain_name)) +
                      " days.", False)
        else:
            Log.error(self, "Your current certificate already expired!", False)

        # CCCSendMail("ccc@{0}".format(ccc_domain_name), ccc_wp_email,
        #  "[FAIL] HTTPS cert renewal {0}".format(ccc_domain_name),
        #          "Hi,\n\nHTTPS certificate renewal for https://{0}
        # was unsuccessful.".format(ccc_domain_name) +
        #           "\nPlease check the CCC CODE log for reason
        # The current expiry date is : " +
        #           str(SSL.getExpirationDate(self, ccc_domain_name)) +
        #           "\n\nFor support visit https://collective-context.org/support .
        # \n\nBest regards,\nYour CCC CODE Worker", files=mail_list,
        #           port=25, isTls=False)
        Log.error(self, "Check the CCC log for more details "
                  "`tail /var/log/ccc/ccc.log`")

    CCCGit.add(self, ["/etc/letsencrypt"],
              msg="Adding letsencrypt folder")
    # CCCSendMail("ccc@{0}".format(ccc_domain_name), ccc_wp_email,
    # "[SUCCESS] Let's Encrypt certificate renewal {0}".format(ccc_domain_name),
    #           "Hi,\n\nYour Let's Encrypt certificate has been renewed for
    # https://{0} .".format(ccc_domain_name) +
...
def setuprocketchat(self):
    if ((not CCCVar.ccc_platform_codename == 'bionic') and
            (not CCCVar.ccc_platform_codename == 'xenial')):
        Log.info(self, "Rocket.chat is only available on Ubuntu 16.04 "
                 "& 18.04 LTS")
        return False
    else:
        if not CCCAptGet.is_installed(self, 'snapd'):
            CCCAptGet.install(self, ["snapd"])
        if CCCShellExec.cmd_exec(self, "snap install rocketchat-server"):
            return True
        return False
