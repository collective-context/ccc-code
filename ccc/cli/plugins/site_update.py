...
class CCCSiteUpdateController(CementBaseController):
    class Meta:
        label = 'update'
        stacked_on = 'site'
        stacked_type = 'nested'
    ...
    def doupdatesite(self, pargs):
        pargs = self.app.pargs
        letsencrypt = False
        for pargs_version in CCCVar.ccc_php_versions:
        ...
        ccc_www_domain = "www.{0}".format(ccc_domain)
        (ccc_domain_type, ccc_root_domain) = CCCDomain.getlevel(
            self, ccc_domain)
        ccc_site_webroot = CCCVar.ccc_webroot + ccc_domain
        ...
        if ((pargs.password or pargs.hsts or
            ...
            if (pargs.password):
                try:
                    updatewpuserpassword(self, ccc_domain, ccc_site_webroot)
            ...
            if (pargs.hsts):
                if pargs.hsts == "on":
                    SSL.setuphsts(self, ccc_domain, enable=True)
                elif pargs.hsts == "off":
                    SSL.setuphsts(self, ccc_domain, enable=False)
            ...
            if (pargs.ngxblocker):
                if pargs.ngxblocker == "on":
                    if os.path.isdir('/etc/nginx/bots.d'):
                        try:
                            setupngxblocker(self, ccc_domain)
                        except SiteError as e:
                            Log.debug(self, str(e))
                    ...
                    try:
                        setupngxblocker(self, ccc_domain, False)
                    except SiteError as e:
                        Log.debug(self, str(e))
                        Log.info(self, "\nngxblocker not enabled.")
            ...
            if (pargs.letsencrypt == 'renew'):
                if CCCAcme.cert_check(self, ccc_domain):
                    if not pargs.force:
                        if (SSL.getexpirationdays(self, ccc_domain) > 30):
                            Log.error(
                                self, "Your cert will expire in more "
                                "than 30 days ( " +
                                str(SSL.getexpirationdays(self, ccc_domain)) +
                                " days).\nAdd \'--force\' to force to renew")
                    Log.wait(self, "Renewing SSL certificate")
                    if CCCAcme.renew(self, ccc_domain):
        ...
        if stype == 'proxy':
            data['site_name'] = ccc_domain
            data['www_domain'] = ccc_www_domain
            data['proxy'] = True
            data['host'] = host
            data['port'] = port
            data['webroot'] = ccc_site_webroot
        ...
        if stype == 'subsite':
            ...
            data = dict(
                site_name=ccc_domain, www_domain=ccc_www_domain,
                static=False, basic=False, multisite=False, webroot=ccc_site_webroot)
            ...
            data["ccc_php"] = ("php" + parent_site_info.php_version).replace(".", "")
            data['subsite'] = True
        ...
        if stype == 'php':
            data = dict(
                ...
                multisite=False, wpsubdir=False, webroot=ccc_site_webroot,
            ...
            data = dict(
                ...
                multisite=False, wpsubdir=False, webroot=ccc_site_webroot,
                ccc_db_name='', ccc_db_user='', ccc_db_pass='',
                ccc_db_host='',
                currsitetype=oldsitetype, currcachetype=oldcachetype)
        ...
        if ((pargs.php74 or pargs.php80 or pargs.php81 or
            ...
            data = dict(
                site_name=ccc_domain,
                www_domain=ccc_www_domain,
        ...
        for pargs_version, version in CCCVar.ccc_php_versions.items():
        ...
        if pargs.letsencrypt:
            ...
            if pargs.letsencrypt == 'on':
                ...
                letsencrypt = True
                acme_subdomain = bool(ccc_domain_type == 'subdomain')
                acme_wildcard = False
        ...
        if all(globals()[version_key] is bool(check_php_version == version) for version_key,
               version in CCCVar.ccc_php_versions.items()) and (stype == oldsitetype
        ...
        for pargs_version, version in CCCVar.ccc_php_versions.items():
            if globals()[pargs_version] is True:
                data['ccc_php'] = pargs_version
                Log.debug(self, f"data ccc_php set to {pargs_version}")
        ...
        ccc_auth = site_package_check(self, stype)
        data['ccc_db_name'] = check_site.db_name
        data['ccc_db_user'] = check_site.db_user
        data['ccc_db_pass'] = check_site.db_password
        data['ccc_db_host'] = check_site.db_host
        ...
        if not (pargs.letsencrypt or pargs.hsts or pargs.ngxblocker):
            ...
            try:
                ...
                Log.info(self, Log.FAIL + "Update site failed."
                         ...
                         "`tail /var/log/ccc/ccc.log` and please try again")
                return 1

        if 'proxy' in data.keys() and data['proxy']:
            updateSiteInfo(self, ccc_domain, stype=stype, cache=cache,
                           ssl=(bool(check_site.is_ssl)))
            Log.info(self, "Successfully updated site"
                     " http://{0}".format(ccc_domain))
            return 0
        ...
        if pargs.letsencrypt:
            if data['letsencrypt'] is True:
                ...
                if CCCAcme.cert_check(self, ccc_domain):
                    ...
                    if acme_subdomain:
                        ...
                        if SSL.checkwildcardexist(self, ccc_root_domain):
                            ...
                            Log.debug(
                                ...
                                .format(ccc_domain))
                            if CCCAcme.setupletsencrypt(
                                    self, acme_domains, acmedata):
                                CCCAcme.deploycert(self, ccc_domain)
                            else:
                                Log.error(
                    ...
                    SSL.httpsredirect(
                        self, ccc_domain, acme_domains, redirect=True)
                    SSL.siteurlhttps(self, ccc_domain)

                if not CCCService.reload_service(self, 'nginx'):
                    Log.error(self, "service nginx reload failed. "
                              "check issues with `nginx -t` command")
                Log.info(self, "Congratulations! Successfully "
                         "Configured SSL on https://{0}".format(ccc_domain))
                letsencrypt = True
                if (SSL.getexpirationdays(self, ccc_domain) > 0):
                ...
                if pargs.letsencrypt == "off":
                    if os.path.islink("{0}/conf/nginx/ssl.conf"
                        ...
                        CCCFileUtils.mvfile(self, "{0}/conf/nginx/ssl.conf"
                                           .format(ccc_site_webroot),
                                           '{0}/conf/nginx/ssl.conf.disabled'
                                           .format(ccc_site_webroot))
                        SSL.httpsredirect(
                            self, ccc_domain, acmedata, redirect=False)
                        if os.path.isfile("{0}/conf/nginx/hsts.conf"
                                          .format(ccc_site_webroot)):
                            CCCFileUtils.mvfile(self, "{0}/conf/nginx/hsts.conf"
                                               .format(ccc_site_webroot),
                                               '{0}/conf/nginx/'
                                               'hsts.conf.disabled'
                                               .format(ccc_site_webroot))
                        # find all broken symlinks
                        sympath = (f'{ccc_site_webroot}/conf')
                        CCCFileUtils.findBrokenSymlink(self, sympath)
            ...
            CCCGit.add(self, ["{0}/conf/nginx".format(ccc_site_webroot)],
                      msg="Adding letsencrypts config of site: {0}"
                      .format(ccc_domain))
            updateSiteInfo(self, ccc_domain, ssl=letsencrypt)
            return 0
        ...
        # if data['ccc_db_name'] and not data['wp']:
        if 'ccc_db_name' in data.keys() and not data['wp']:
            try:
                ...
                return 1
            try:
                cccdbconfig = open("{0}/ccc-config.php".format(ccc_site_webroot),
                                  encoding='utf-8', mode='w')
                cccdbconfig.write("<?php \ndefine('DB_NAME', '{0}');"
                                 "\ndefine('DB_USER', '{1}'); "
                ...
                Log.debug(self, str(e))
                Log.debug(self, "creating ccc-config.php failed.")
                Log.info(self, Log.FAIL + "Update site failed. "
                         "Check the log for details: "
                         "`tail /var/log/ccc/ccc.log` and please try again")
                return 1
        ...
        if data['wp'] and oldsitetype in ['html', 'proxy', 'php', 'php72',
            ...
            try:
                ccc_wp_creds = setupwordpress(self, data)
            except SiteError as e:
                Log.debug(self, str(e))
                Log.info(self, Log.FAIL + "Update site failed."
                         "Check the log for details: "
        ...
        CCCGit.add(self, ["/etc/nginx"],
                  ...
                  .format(ccc_www_domain, stype, cache))
        ...
        if data['wp'] and oldsitetype in ['html', 'php', 'mysql']:
            Log.info(self, "\n\n" + Log.ENDC + "WordPress admin user :"
                     " {0}".format(ccc_wp_creds['wp_user']))
            Log.info(self, Log.ENDC + "WordPress admin password : {0}"
                     .format(ccc_wp_creds['wp_pass']) + "\n\n")
        if oldsitetype in ['html', 'php'] and stype != 'php':
            updateSiteInfo(self, ccc_domain, stype=stype, cache=cache,
                           db_name=data['ccc_db_name'],
                           db_user=data['ccc_db_user'],
                           db_password=data['ccc_db_pass'],
                           db_host=data['ccc_db_host'],
                           ssl=bool(check_site.is_ssl),
                           php_version=check_php_version)
        else:
            updateSiteInfo(self, ccc_domain, stype=stype, cache=cache,
