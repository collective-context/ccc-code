...
class SSL:
    ...
    def getexpirationdays(self, domain, returnonerror=False):
        ...
        Log.debug(
            ...
            .format(domain))
        current_date = CCCShellExec.cmd_exec_stdout(self, "date -d \"now\" +%s")
        expiration_date = CCCShellExec.cmd_exec_stdout(
    ...
    def getexpirationdate(self, domain):
        ...
        return CCCShellExec.cmd_exec_stdout(
    ...
    def siteurlhttps(self, domain):
        ccc_site_webroot = ('/var/www/{0}'.format(domain))
        CCCFileUtils.chdir(
            self, '{0}/htdocs/'.format(ccc_site_webroot))
        if CCCShellExec.cmd_exec(
                self, "{0} --allow-root core is-installed"
                .format(CCCVar.ccc_wpcli_path)):
            ccc_siteurl = (
                CCCShellExec.cmd_exec_stdout(
                    self, "{0} option get siteurl "
                    .format(CCCVar.ccc_wpcli_path) +
                    "--allow-root --quiet"))
            test_url = re.split(":", ccc_siteurl)
            if not (test_url[0] == 'https'):
                ...
                try:
                    CCCShellExec.cmd_exec(
                        ...
                        "\'https://{1}\' --allow-root"
                        .format(CCCVar.ccc_wpcli_path, domain))
                    CCCShellExec.cmd_exec(
    ...
    def checkwildcardexist(self, ccc_domain_name):
        ...
        CCCShellExec.cmd_exec(
            self, "{0} ".format(ccc_acme_exec) +
            "--list --listraw > /var/lib/ccc/cert.csv")

        # define new csv dialect
        csv.register_dialect('acmeconf', delimiter='|')
        # open file
        certfile = open('/var/lib/ccc/cert.csv', mode='r', encoding='utf-8')
        reader = csv.reader(certfile, 'acmeconf')
        ccc_wildcard_domain = ("*.{0}".format(ccc_domain_name))
        for row in reader:
            if ccc_wildcard_domain == row[2]:
                if not row[3] == "":
                    return True
        certfile.close()
        return False

    def setuphsts(self, ccc_domain_name, enable=True):
        """Enable or disable htsts for a site"""
        if enable is True:
            if CCCFileUtils.enabledisable(
                self, f'/var/www/{ccc_domain_name}/conf/nginx/hsts.conf'
            ):
                return 0
            else:
                Log.info(
                ...
                hstsconf = open(f"/var/www/{ccc_domain_name}/conf/nginx/hsts.conf",
    ...
    def selfsignedcert(self, proftpd=False, backend=False):
        ...
        selfs_tmp = '/var/lib/ccc/tmp/selfssl'
        # create self-signed tmp directory
        if not os.path.isdir(selfs_tmp):
            CCCFileUtils.mkdir(self, selfs_tmp)
        try:
            CCCShellExec.cmd_exec(
        ...
        CCCFileUtils.rm(self, selfs_tmp)

    def httpsredirect(self, ccc_domain_name, acme_domains, redirect=True):
        """Create Nginx redirection from http to https"""
        ccc_acme_domains = ' '.join(acme_domains)
        data = dict(domains=ccc_acme_domains)
        if redirect:
            Log.wait(self, "Adding HTTPS redirection")
            if CCCFileUtils.enabledisable(
                    self, '/etc/nginx/conf.d/force-ssl-{0}.conf'
                    .format(ccc_domain_name), enable=True):
                Log.valide(self, "Adding HTTPS redirection")
                return 0
            else:
                try:
                    CCCTemplate.deploy(self,
                                      f'/etc/nginx/conf.d/force-ssl-{ccc_domain_name}.conf',
                                      'force-ssl.mustache',
                                      data)
                except IOError as e:
                    Log.debug(self, str(e))
            ...
            if CCCFileUtils.enabledisable(
                    self, "/etc/nginx/conf.d/force-ssl-{0}.conf"
                    .format(ccc_domain_name), enable=False):
                Log.info(
                    self, "Disabled HTTPS Force Redirection for site "
                    "{0}".format(ccc_domain_name))
            else:
                Log.info(
    ...
    def archivedcertificatehandle(self, domain, acme_domains):
        Log.warn(
            ...
            "(ref: {0}/"
            "{1}_ecc/{1}.conf)".format(CCCVar.ccc_ssl_archive, domain) +
        ...
        if check_prompt == "1":
            ...
            ssl = CCCAcme.deploycert(self, domain)
            if ssl:
            ...
            if ssl:
                CCCAcme.deploycert(self, domain)
        else:
        ...
        if os.path.isfile("{0}/conf/nginx/ssl.conf"
            ...
            CCCFileUtils.mvfile(self, "/var/www/{0}/conf/nginx/ssl.conf"
                               .format(domain),
